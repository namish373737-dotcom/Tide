import { View, Text, Pressable, ScrollView, StyleSheet, Alert, StatusBar, TextInput, ActivityIndicator } from 'react-native';
...
export default function ReportScreen() {
  ...
  const generatePDF = async () => {
    ...
    setGenerating(true);
    try {
      const html = buildHTML(entries, medications, days, patientName, appointmentWith);
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.card}>
          <Text style={styles.cardLabel}>Patient Details (optional)</Text>
          <TextInput
            value={patientName}
          ...
          <TextInput
            ...
            onChangeText={setAppointmentWith}
            placeholder="Preparing for appointment with (e.g. Dr. Smith)"
            placeholderTextColor={COLORS.textTertiary}
        ...
        <Pressable onPress={generatePDF} disabled={generating || loading} style={({ pressed }) => [styles.generateButton, (pressed || generating || loading) && styles.generateButtonPressed]}>
          {generating ? <ActivityIndicator color={COLORS.white} /> : <FileText size={18} color={COLORS.white} />}
          <Text style={styles.generateButtonText}>{generating ? 'Generating…' : 'Generate PDF'}</Text>
        </Pressable>
...
function buildHTML(entries: any[], medications: any[], days: number, patientName = '', appointmentWith = ''): string {
  ...
  return `<html>
    ...
    <body>
      ...
      ${appointmentWith ? `<p><strong>Appointment:</strong> ${escapeHtml(appointmentWith)}</p>` : ''}
...
const styles = StyleSheet.create({
  ...
  nameInput: { height: 44, backgroundColor: COLORS.surfaceElevated, borderRadius: RADIUS.lg, paddingHorizontal: SPACING.base, ...TYPOGRAPHY.body, color: COLORS.text, borderWidth: 1, borderColor: COLORS.border },
});
